...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      urldownload() {
        return document.location.protocol + "//" + window.location.host + "/jeecg-boot/api/" + "appVersions/downloadApk/" + this.appInfo.currentVersion.id;
      },
    },
    mounted() {
      this.getAppInfo(this.$route.params.code);
    ...
    methods: {
      getAppInfo(shortCode) {
        ...
        this.getAction(`/api/apps/shortCode/${shortCode}`, {}).then(res => {
          if (res.success) {
      ...
      download() {
        let url = "http://127.0.0.1:8080/jeecg-boot/api/" + "appVersions/downloadApk/" + this.appInfo.currentVersion.id;
